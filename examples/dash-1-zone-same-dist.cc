...
int main(int argc, char *argv[]) {
    ...
    for (size_t i = 0; i < 360; i += aux) {
        ...
        std::cout << "(x - y) = " << dist*cosseno << " - " << dist*seno << '\n';
    }

    positionAlloc ->Add(Vector(16, 16, 0)); // node0
    positionAlloc ->Add(Vector(14, 14, 0)); // node0
    positionAlloc ->Add(Vector(12, 12, 0)); // node0
    positionAlloc ->Add(Vector(10, 10, 0)); // node0
    ...
    AnimationInterface anim(dir + std::string("/") + flowfile + std::string(".netanim"));
    ...
    Simulator::Run();
  	...
  	monitor->SerializeToXmlFile (dir + std::string("/") + flowfile + std::string(".flowmon"), true, true);
...
void calcAvg(uint32_t &tNodes, uint32_t &layer, uint32_t &users) {
    ...
    arq << dir << "/" << "PB_total_" << tNodes << "_" << users << "_" << layer << ".txt";
